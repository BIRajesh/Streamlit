...
def create_prompt(myquestion,rag):
    if rag ==1:
        ...
        df_context = session.sql(query,params=[myquestion,num_chunks]).to_pandas()
        loop_count = len(df_context)-1
        promp_text =""
        for i in range(0,loop_count):
            promp_text += df_context._get_value(i,'CHUNK')
        promp_text = promp_text.replace("'", "")
